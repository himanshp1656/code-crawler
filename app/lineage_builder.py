...
def build_lineage(
    ...
    Each function (asset) will have:
        ...
        - file: file path
        - lineno: line number
        - upstream_ids: IDs of functions that call this function
    """
...
# create assets
    for fn in func_index.values():
        asset_id = f"{workflow_id}/{run_id}/{fn.id}"
        assets[asset_id] = {
            "id": asset_id,
            "name": f"{workflow_id}/{run_id}/{fn.qualname}",
            "file": fn.file,
            "lineno": fn.lineno,
            "upstream_ids": [],
        }
    ...
    for file_result in files.values():
        for call in file_result.calls:
            ...
            caller_asset_id = f"{workflow_id}/{run_id}/{call.caller_id}"
            callee_asset_id = f"{workflow_id}/{run_id}/{callee_id}"

            if callee_asset_id in assets:
                assets[callee_asset_id]["upstream_ids"].append(caller_asset_id)
